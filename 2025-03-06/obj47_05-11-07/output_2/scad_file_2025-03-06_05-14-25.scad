...
// Parameters
hub_diameter = 30;
hub_height = 10; // Ensuring two hubs are present
bore_diameter = 10;
fastening_hole_diameter = 6; // Enlarged to match the original model
fastening_hole_offset = 10; // Adjusted hole positions
slit_width = 2;
slit_depth = hub_diameter / 2; // Extended slit depth
spacer_thickness = 4; // Increased thickness for better flexibility
spacer_wavy_amplitude = 3; // More pronounced wavy shape
spacer_wavy_frequency = 6; // More distinct wavy pattern
screw_diameter = 4;
screw_length = 25; // Extended to pass through both hubs and spacer
...
module flexible_coupling() {
    // Top hub
    translate([0, 0, hub_height + spacer_thickness/2])
        hub();
    
    // Fastening screws (top layer, correctly positioned)
    for (i = [0:3]) {
        rotate([0, 0, i * 90])
            translate([fastening_hole_offset, 0, hub_height + spacer_thickness/2])
            fastening_screw();
    }
    ...
    // Fastening screws (middle layer, correctly positioned)
    for (i = [0:3]) {
        rotate([0, 0, i * 90])
            translate([fastening_hole_offset, 0, spacer_thickness/2])
            fastening_screw();
    }
