...
// Parameters
shaft_diameter = 20;
shaft_length = 160; // Ensuring proper connection between hubs
hub_diameter = 50;
hub_height = 30;
flange_diameter = 60;
flange_thickness = 3; // Adjusted to match original model
spacer_diameter = 55;
spacer_thickness = 3;
bolt_diameter = 5;
bolt_length = 30; // Adjusted for proper fastening
nut_diameter = 8;
nut_thickness = 4;
bolt_hole_radius = 25;
num_bolts = 6;
...
module coupling() {
    ...
    // Flange plates and spacers on input side
    ...
    // Flange plates and spacers on output side
    translate([0, 0, -shaft_length / 2 + flange_thickness + spacer_thickness]) flange_plate();
    translate([0, 0, -shaft_length / 2 + flange_thickness]) spacer_ring();
    translate([0, 0, -shaft_length / 2]) flange_plate();
    ...
    for (i = [0:num_bolts-1]) {
        ...
        // Adding nuts to secure bolts
        translate([bolt_hole_radius * cos(angle), bolt_hole_radius * sin(angle), shaft_length / 2 + bolt_length / 2])
            nut();
        translate([bolt_hole_radius * cos(angle), bolt_hole_radius * sin(angle), -shaft_length / 2 - bolt_length / 2])
            nut();
    }
