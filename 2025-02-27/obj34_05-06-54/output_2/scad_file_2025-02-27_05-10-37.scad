...
// Parameters
hub_diameter = 50;
hub_height = 50; // Extended height for more refined shape
hub_hole_diameter = 25; // More pronounced central hole
flange_diameter = 80;
flange_thickness = 2.5; // Reduced thickness for accuracy
spacer_thickness = 6; // More distinct spacer ring
bolt_diameter = 3; // Reduced bolt size
bolt_length = 45;
num_bolts = 24; // Increased number of bolts for better distribution
bolt_circle_diameter = 72; // Adjusted for better alignment
    ...
    difference() {
        ...
        for (i = [0:6]) {
            ...
            translate([hub_diameter/2 * cos(angle), hub_diameter/2 * sin(angle), 0])
                rotate([0, 0, angle])
                cube([6, 2.5, hub_height], center = true);
        }
    }
}
