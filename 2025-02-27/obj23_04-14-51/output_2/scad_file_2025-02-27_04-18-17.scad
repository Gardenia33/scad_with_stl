...
// Parameters
shaft_diameter = 20;
shaft_length = 80; // Extended to ensure a clear central shaft
hub_diameter = 50;
hub_length = 30;
flange_diameter = 60;
flange_thickness = 5;
...
bolt_diameter = 5;
bolt_length = 10; // Shortened bolts to match the original model
nut_diameter = 8;
nut_thickness = 4;
bolt_hole_radius = 25;
num_bolts = 4;
...
module coupling() {
    ...
    // Left Flange Plates and Spacer Ring
    ...
    // Right Flange Plates and Spacer Ring
    translate([0, 0, -shaft_length / 2 + hub_length + flange_thickness / 2]) flange_plate();
    translate([0, 0, -shaft_length / 2 + hub_length + flange_thickness + spacer_ring_thickness / 2]) spacer_ring();
    translate([0, 0, -shaft_length / 2 + hub_length + flange_thickness + spacer_ring_thickness + flange_thickness / 2]) flange_plate();
